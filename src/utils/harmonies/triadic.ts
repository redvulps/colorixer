...
export function getTriadicColors(baseColor: HSL): string[] {
  ...
  const color2 = chroma.hsl((baseColor.h + 120) % 360, baseColor.s, baseColor.l);
  const color3 = chroma.hsl((baseColor.h + 240) % 360, baseColor.s, baseColor.l);

  return [color1.hex(), color2.hex(), color3.hex()];
}
